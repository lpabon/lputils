...
import (
	...
	hcapi "github.com/lpabon/lputils/healthcheck/api"
)
...
type Reporter interface {
	...
	ToHealthCheckResults() *hcapi.HealthCheckResults
...
func (cr *SimpleReporter) ToHealthCheckResults() *hcapi.HealthCheckResults {
	var categories []*hcapi.HealthCheckCategory

	collectOutput := func(result *CheckResult) {
		if categories == nil || categories[len(categories)-1].Name != string(result.Category) {
			categories = append(categories, &hcapi.HealthCheckCategory{
				Name:   string(result.Category),
				Checks: []*hcapi.HealthCheck{},
			})
		}
		...
		if !result.Retry {
			...
			var status hcapi.HealthCheckResultTypes_Type
			if result.Err == nil {
				status = hcapi.HealthCheckResultTypes_SUCCESS
			} else if result.Warning {
				status = hcapi.HealthCheckResultTypes_WARNING
			} else {
				status = hcapi.HealthCheckResultTypes_ERROR
			}

			currentCheck := &hcapi.HealthCheck{
	...
	return &hcapi.HealthCheckResults{
		Success:    success,
		Warning:    warning,
